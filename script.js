...
const FOOD_GROUPS = { "主食":1,"乳製品":2,"卵・大豆製品":3,"野菜":4,"果物":5,"肉・魚":6,"肉":6,"魚介":6 };
...
fetch('foods.json')
  .then(res => res.json())
  .then(data => { FOODS=data; populateCategory(); renderFoodList(FOODS); })
...
function filterFoods(){
  ...
  renderFoodList(FOODS.filter(f => (category==='all'||f.category===category)&&f.name.toLowerCase().includes(keyword)));
}
...
function updateGrams(index, grams){ state.selected[index].grams=parseFloat(grams); renderSelected(); }
function removeFood(index){ state.selected.splice(index,1); renderSelected(); }

function updateTotals(){
  const totals={energy:0, protein:0, fat:0, carb:0, calcium:0, iron:0, vitaminA:0, vitaminC:0, salt:0};
  state.selected.forEach(f=>{ totals.energy+=f.energy*f.grams/100; totals.protein+=f.protein*f.grams/100; totals.fat+=f.fat*f.grams/100; totals.carb+=f.carb*f.grams/100; totals.calcium+=f.calcium*f.grams/100; totals.iron+=f.iron*f.grams/100; totals.vitaminA+=f.vitaminA*f.grams/100; totals.vitaminC+=f.vitaminC*f.grams/100; totals.salt+=f.salt*f.grams/100; });
...
function updateChart(){
  ...
  chart=new Chart(ctx,{
    ...
    data:{
      ...
      datasets:[
        { label:'合計栄養素', data:[totals.energy,totals.protein,totals.fat,totals.carb,totals.calcium,totals.iron,totals.vitaminA,totals.vitaminC,totals.salt], backgroundColor:'rgba(54,162,235,0.7)', borderColor:'rgba(54,162,235,1)', borderWidth:1 },
        { label:'目標値', data:[goal.energy,goal.protein,goal.fat,goal.carb,goal.calcium,goal.iron,goal.vitaminA,goal.vitaminC,goal.salt], type:'line', borderColor:'rgba(255,99,132,0.7)', borderWidth:2, borderDash:[5,5], fill:false, tension:0.3 }
      ]
    },
